...
const ProjectDonateCard = ({
  ...
  useEffect(() => {
    setIsAdmin(compareAddresses(adminUser?.walletAddress, user?.walletAddress));
  }, [user, adminUser]);
  ...
  return (
    <>
      {showVerificationModal && (
        <VerificationModal closeModal={() => setShowVerificationModal(false)} />
      )}
      {showModal && slug && (
        ...
        {isAdmin ? (
          <>
            ...
            <FullButton
              ...
              onClick={() => router.push(idToProjectEdit(project?.id || ""))}
            />
            {/* {!verified && !isDraft && (
							<FullOutlineButton
								buttonType='primary'
